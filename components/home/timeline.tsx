...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  const addText = (
    ...
    const titleSizeClass = size === ItemSize.LARGE ? "lg:text-5xl md:text-2xl sm:text-xl seq section-heading" : "lg:text-3xl md:text-xl sm:text-lg";
    const logoString = image
      ? `<Image src='${image}' class='h-8 mb-2' loading='lazy' width='100' height='32' alt='${image}' />`
      : "";
    const subtitleString = subtitle
      ? `<p class='lg:text-xl md:text-lg sm:text-sm mt-2 text-gray-200 font-medium tracking-wide'>${subtitle}</p>`
      : "";
  ...
  const renderSlides = (): React.ReactNode => (
    <div className="max-w-full h-96 md:h-30 md:w-30 mx-auto shadow-xl bg-gray-800 rounded-2xl overflow-hidden" ref={screenContainer}>
      <Image
        className="w-full h-8 md:w-96 md:h-34"
        src="/timeline/black.jpg"
        alt="Title bar"
        width={644}
        height={34}
      />
      <div className="relative h-full w-full -mt-2">
        <div className="absolute top-0 left-0 h-full w-full">
          {svgCheckpointItems.map((item, index) => (
            <Image
              className={`w-full md:w-96 absolute top-0 object-cover slide-${index + 1}`}
  ...
  const renderSectionTitle = (): React.ReactNode => (
    <div className="flex flex-col">
      ...
      <h1 className="section-heading seq mt-2">Why Us?</h1>
      <h2 className="text-2xl md:max-w-2xl w-full seq mt-2">
        Our mission is to Digitalize World and Automate Work for more productivity. See How we can help you.
      </h2>
    </div>
  );
